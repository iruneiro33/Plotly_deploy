...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    // 1. Create a variable that filters the metadata array for the object with the desired sample number (guage code)
    var metadataArray = data.metadata.filter(chartObj => chartObj.id == sample);

    // 5. Create a variable that holds the first sample in the array.
    var chartResult = chartArray[0];

    // 2. Create a variable that holds the first sample in the metadata array (guage code)
    ...
    // 3. Create a variable that holds the washing frequency (guage code)
